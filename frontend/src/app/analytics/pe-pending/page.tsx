...
import { FileText, RefreshCw, Download, Search, X, ChevronDown } from "lucide-react";
...
export default function PEPendingPage() {
  ...
  const { data: peData } = usePEAnalysis({ page: 1, per_page: perPage, valuation_filter: "pending", ...allFilters });
  ...
  const handleFetchCMP = async () => {
    try {
      const result = await triggerJob("fetch_quotes");
      toast.success(`CMP fetch started: ${result.job_id || "queued"}`);
    } catch {
  ...
  const handleExport = async () => {
    try {
      const data = await exportPEAnalysisCSV({ ...allFilters, valuation_filter: "pending" });
      const rows = data.results || [];
      if (!rows.length) { toast.error("No data to export"); return; }
      const headers = ["stock_symbol", "company_name", "quarter", "financial_year", "exchange", "eps_diluted_standalone", "eps_diluted_consolidated", "fy_eps_diluted_standalone", "fy_eps_diluted_consolidated", "manual_fy_eps", "cmp", "pe", "recommendation", "target_price", "valuation", "extraction_status"];
      const headerLabels = ["Symbol", "Company", "Quarter", "Year", "Exchange", "Qtr EPS (S)", "Qtr EPS (C)", "FY EPS (S)", "FY EPS (C)", "Manual EPS", "CMP", "PE", "Signal", "Target", "Valuation", "Status"];
      ...
      a.href = url;
      a.download = `pe_pending_${filters.year || "all"}_${filters.quarter || "all"}.csv`;
      a.click();
  ...
  return (
    <div className="space-y-2 h-full flex flex-col">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-semibold text-gray-900">PE Pending — Q4 FY26 Results</h2>
      </div>
      ...
      <div className="flex-1 min-h-0 bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
        <PETable valuationFilter="pending" filters={allFilters} perPage={perPage} visibleColumns={visibleCols} />
      </div>
    </div>
  );
}
