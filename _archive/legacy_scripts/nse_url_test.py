...
async def lifespan(app: FastAPI):
    ...
    # Initialize databases (after sector_map loaded — one-time migration uses it)
    await init_db()
    await init_analytics_db()
    logger.info("Databases initialized (messages.db + analytics.db)")

    global _msg_flush_task
    _msg_flush_task = asyncio.create_task(_msg_write_flusher())
    logger.info("Message write batcher started (flush every 500ms)")
    ...
    bse_board_meeting_task = asyncio.create_task(run_periodic_task_bse_board_meeting())
    extraction_logger_task = asyncio.create_task(run_extraction_status_logger())
    cleanup_task = asyncio.create_task(run_periodic_cleanup())
    
    scheduled_quotes_task = None
    if AUTO_FETCH_ENABLED:
        scheduled_quotes_task = asyncio.create_task(run_scheduled_fetch_quotes())
        logger.info("All background tasks started: NSE, BSE-All, BSE-Results, BSE-BoardMeeting, Periodic Cleanup, Scheduled Fetch Quotes")
        ...
        logger.info("Background tasks started: NSE, BSE-All, BSE-Results, BSE-BoardMeeting, Periodic Cleanup")
    ...
    if _msg_flush_task and not _msg_flush_task.done():
        _msg_flush_task.cancel()
        try:
            await _msg_flush_task
        except asyncio.CancelledError:
            pass
...
# Database setup - split into two files to avoid write contention
# messages.db: high-write tables (messages, sessions, users, scheduled_fetch_config)
# analytics.db: PE analysis tables (quarterly_results, stocks, failed_extractions, pe_formulas, sector_formulas)
...
@asynccontextmanager
async def open_db():
    """Messages DB — dashboard messages, auth, config."""
    async with aiosqlite.connect(DB_PATH) as db:
        for p in _DB_PRAGMAS:
            await db.execute(p)
        yield db

@asynccontextmanager
async def open_analytics_db(readonly: bool = False):
    """Analytics DB — quarterly_results, stocks, pe_formulas, sector_formulas, failed_extractions.
    readonly=True sets PRAGMA query_only=ON so reads never wait for the write lock in WAL mode."""
    async with aiosqlite.connect(ANALYTICS_DB_PATH) as db:
        for p in _DB_PRAGMAS:
            await db.execute(p)
        if readonly:
...
# ---------------------------------------------------------------------------
# Message write batcher — accumulates INSERT INTO messages for 500ms, then
# flushes all rows in a single transaction (1 lock acquisition instead of N).
# ---------------------------------------------------------------------------
_msg_write_queue: asyncio.Queue = asyncio.Queue()
_msg_flush_task: asyncio.Task = None
_MSG_FLUSH_INTERVAL = 0.5  # seconds

async def _msg_write_flusher():
    """Background loop: drain the queue every 500ms and flush in one transaction."""
    while True:
        await asyncio.sleep(_MSG_FLUSH_INTERVAL)
        batch = []
        while not _msg_write_queue.empty():
            try:
                ...
                for item in batch:
                    cursor = await db.execute(
                        """INSERT INTO messages
                           (chat_id, message, timestamp, symbol, company_name,
                            description, file_url, raw_message, option, sector, exchange)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                        item["params"],
                    )
                    if item["future"] and not item["future"].done():
                        item["future"].set_result(cursor.lastrowid)
                await db.commit()
            for item in batch:
                if item["ws_msg"]:
                    try:
                        await ws_manager.broadcast_message(item["ws_msg"])
                    except Exception:
                        pass
            if len(batch) > 1:
                logger.debug(f"Message batcher: flushed {len(batch)} rows in 1 txn")
        except Exception as e:
            logger.error(f"Message batcher flush error: {e}")
            for item in batch:
                if item["future"] and not item["future"].done():
                    item["future"].set_exception(e)


async def _enqueue_message_write(params: tuple, ws_msg: dict = None) -> int:
    """Push a message INSERT into the batch queue. Returns message_id once flushed."""
    future = asyncio.get_event_loop().create_future()
    await _msg_write_queue.put({"params": params, "ws_msg": ws_msg, "future": future})
    return await future
...
async def _auto_fetch_cmp_for_stock(symbol: str, fy_eps: float, exchange: str = "NSE") -> Dict:
    ...
    try:
        ...
        async with open_analytics_db() as db:
            await db.execute(
...
async def init_db():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        wal_result = await db.execute("PRAGMA journal_mode=WAL")
        wal_mode = (await wal_result.fetchone())[0]
        ...
        # Migration: add new columns to stocks
        for col, col_type in [
            ("nse_token", "INTEGER"),
            ("bse_token", "INTEGER"),
            ("isin", "TEXT"),
        ...
        if migrate_needed:
            ...
            safe_cols = [c for c in existing_cols if c in [
                ...
                "manual_fy_eps", "manual_fy_eps_formula",
            ]]
        ...
        await db.execute("CREATE INDEX IF NOT EXISTS idx_qr_sym_fy_q ON quarterly_results(stock_symbol, financial_year DESC, quarter DESC)")
        await db.execute("CREATE INDEX IF NOT EXISTS idx_qr_valuation ON quarterly_results(valuation)")
        await db.execute("CREATE INDEX IF NOT EXISTS idx_qr_created_at ON quarterly_results(created_at DESC)")
        await db.execute("CREATE INDEX IF NOT EXISTS idx_qr_quarter_val ON quarterly_results(quarter, valuation)")
        await db.execute("""CREATE INDEX IF NOT EXISTS idx_qr_quarters_cover ON quarterly_results(
        ...
        # Sector-level formula overrides (priority: sub_sector > sector > global default)
        await db.execute("""
            CREATE TABLE IF NOT EXISTS sector_formulas (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ...
                created_at TEXT NOT NULL,
                updated_at TEXT NOT NULL,
                UNIQUE(sector, sub_sector, quarter)
            )
        """)
...
async def init_analytics_db():
    """Initialize analytics.db — separate file from messages.db to eliminate write-lock contention.
    On first run, migrates existing data from messages.db via ATTACH."""
    is_new = not os.path.exists(ANALYTICS_DB_PATH)
    async with aiosqlite.connect(ANALYTICS_DB_PATH) as db:
        ...
        await db.execute("""
            CREATE TABLE IF NOT EXISTS bse_announcements_log (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ...
                created_at TEXT NOT NULL,
                processed_at TEXT,
                UNIQUE(scrip_code, announcement_type, pdf_url)
            )
        """)
        await db.execute("CREATE INDEX IF NOT EXISTS idx_bse_ann_scrip_date ON bse_announcements_log(scrip_code, announcement_date)")
        await db.execute("CREATE INDEX IF NOT EXISTS idx_bse_ann_processed ON bse_announcements_log(processed)")

        # One-time migration: copy existing data from messages.db → analytics.db
        if is_new and os.path.exists(DB_PATH):
            logger.info("First run: migrating analytics data from messages.db → analytics.db ...")
            try:
                await db.execute("ATTACH DATABASE ? AS msg_db", (DB_PATH,))
                for tbl in ["stocks", "quarterly_results", "failed_extractions", "pe_formulas", "sector_formulas"]:
                    try:
                        cnt_cur = await db.execute(f"SELECT COUNT(*) FROM msg_db.{tbl}")
                        cnt = (await cnt_cur.fetchone())[0]
                        if cnt > 0:
                            await db.execute(f"INSERT OR IGNORE INTO {tbl} SELECT * FROM msg_db.{tbl}")
                            logger.info(f"  Migrated {cnt} rows from msg_db.{tbl}")
                    except Exception as e:
                        logger.debug(f"  Skip migration for {tbl}: {e}")
                await db.execute("DETACH DATABASE msg_db")
            except Exception as e:
                logger.warning(f"Migration from messages.db failed (non-fatal): {e}")

        qr_cur = await db.execute("SELECT COUNT(*) FROM quarterly_results")
        logger.info(f"analytics.db quarterly_results: {(await qr_cur.fetchone())[0]} rows")

        # Stocks master sync — ATTACH messages.db so sync can read messages table
        ...
        await db.commit()
    logger.info("Analytics database initialized (analytics.db)")
                ...
                if not os.path.exists(session_file):
                    ...
                    try:
                        from get_quote import (
                            ...
                            get_quotes_with_rate_limit, retry_missing_quotes
                        )
                        ...
                        # Retry stocks that returned open=0
                        missing_count = sum(1 for idx in valid_indices if pd.isna(df.at[idx, 'OPEN PRICE']))
                        if missing_count > 0:
                            await ws_manager.broadcast_message({
                                "type": "scheduled_task",
                                "status": "progress",
                                "task": "fetch_quotes",
                                "progress": 80,
                                "message": f"📊 Retrying {missing_count} stocks with no price...",
                                "timestamp": get_ist_now().isoformat()
                            })
                            df, retried_count = await retry_missing_quotes(df, valid_indices, max_retries=2, retry_delay=120)
                            logger.info(f"Scheduled retry filled {retried_count} additional prices")
                        ...
                        final_missing = sum(1 for idx in valid_indices if pd.isna(df.at[idx, 'OPEN PRICE']))
                        logger.info(f"✅ Scheduled fetch quotes completed - {total_symbols} stocks processed ({final_missing} still missing)")
                        scheduled_fetch_logger.info(f"COMPLETED - {get_ist_now().strftime('%Y-%m-%d %H:%M:%S')} IST - {total_symbols} stocks processed, {final_missing} missing")
...
async def send_webhook_message(chat_id: int, text: str):
    if not TELEGRAM_ENABLED:
        logger.debug(f"[Telegram OFF] Skipped send_webhook_message to {chat_id}")
        return
...
async def trigger_watchlist_message(message):
    if not TELEGRAM_ENABLED:
        logger.debug(f"[Telegram OFF] Skipped trigger_watchlist_message")
        return
...
async def save_announcement_to_dashboard(symbol, company_name, description, file_url, exchange="NSE", option="all", message=""):
    ...
    try:
        ...
        params = (
            message_data.chat_id, message_data.message, message_data.timestamp,
            message_data.symbol, message_data.company_name, message_data.description,
            message_data.file_url, message_data.message, message_data.option,
            message_data.sector or "", message_data.exchange or "NSE"
        )
        ws_msg = {"type": "new_message", "message": message_data.dict()}
        message_id = await _enqueue_message_write(params, ws_msg)
...
async def trigger_test_message(chat_idd, message, type="test", symbol="", company_name="", description="", file_url="", exchange="NSE", save_to_dashboard=True):
    if TELEGRAM_ENABLED:
        ...
        logger.debug(f"[Telegram OFF] Skipped trigger_test_message to {chat_idd}")
    ...
    try:
        ...
        if type == "test":
            if TELEGRAM_ENABLED:
                logger.info(f"Test message sent to Telegram (not saved to DB): {message_data.symbol} - {message_data.company_name}")
            return None
        ...
        ws_msg = {"type": "new_message", "message": message_data.dict()}
        message_id = await _enqueue_message_write(params, ws_msg)
        logger.info(f"Message saved to dashboard: {message_data.symbol} - {message_data.company_name}")
        return message_id
...
async def _get_sector_formulas_for_stock(db, stock_symbol: str) -> Dict[str, str]:
    """Lookup sector_formulas for a stock. Returns {Q1: expr, Q2: expr, ...} or empty dict.
    Priority: sub_sector match > sector-only match."""
    cursor = await db.execute(
        "SELECT sector, sub_sector FROM stocks WHERE symbol = ?", (stock_symbol,)
    )
    row = await cursor.fetchone()
    if not row or not row[0]:
        return {}
    sector = row[0]
    sub_sector = (row[1] or '').strip() if len(row) > 1 else ''

    # Try sub_sector match first, then sector-only (empty string)
    candidates = [sub_sector, ''] if sub_sector else ['']
    for sub_sec in candidates:
        fc = await db.execute(
            "SELECT quarter, formula_expr FROM sector_formulas WHERE sector = ? AND sub_sector = ?",
            (sector, sub_sec)
        )
        frows = await fc.fetchall()
        if frows:
            return {r[0]: r[1] for r in frows}
    return {}


def _eval_formula_expr(expr: str, variables: Dict[str, float]) -> float:
    """Safely evaluate a formula expression like 'Q1*4' or '(Q1+Q2)*2' with given variables."""
    import re
    safe_expr = expr.strip()
    # Replace variable names with values (longest first to avoid partial matches like PQ1 vs Q1)
    for var in sorted(variables.keys(), key=len, reverse=True):
        val = variables[var]
        if val is not None:
            safe_expr = safe_expr.replace(var, str(val))
    # Only allow digits, math operators, parens, dots, whitespace
...
async def _insert_extraction_placeholder(stock_symbol, company_name, pdf_url, exchange, announcement_date, message_id=None, status='queued', error_msg=None):
    ...
    Skips if a completed extraction with valid EPS already exists for same stock+quarter+day."""
    try:
        now = get_ist_now()
        ann_date = announcement_date if announcement_date else now.isoformat()
        quarter, fy = _quarter_fy_from_date(ann_date)
        now_iso = now.isoformat()
        actual_pdf = pdf_url if pdf_url and pdf_url != '-' else None
        ann_date_only = str(ann_date)[:10]
        async with open_analytics_db() as db:
            existing = await db.execute("""
                SELECT 1 FROM quarterly_results
                WHERE stock_symbol = ? AND quarter = ? AND financial_year = ?
                  AND date(announcement_date) = ?
                  AND extraction_status = 'completed'
                  AND (eps_diluted_consolidated IS NOT NULL OR eps_diluted_standalone IS NOT NULL
                       OR eps_basic_consolidated IS NOT NULL OR eps_basic_standalone IS NOT NULL)
            """, (stock_symbol, quarter, fy, ann_date_only))
            if await existing.fetchone():
                logger.info(f"Skipping placeholder for {stock_symbol} ({quarter} {fy}) — completed extraction with EPS already exists")
                return
            await db.execute("""
...
async def _upsert_quarterly_result(db, stock_symbol, company_name, quarter, financial_year,
    ...
    await db.execute("""
        ...
        DO UPDATE SET
            company_name = COALESCE(excluded.company_name, quarterly_results.company_name),
            period_ended = COALESCE(excluded.period_ended, quarterly_results.period_ended),
            ...
            source_pdf_url = COALESCE(excluded.source_pdf_url, quarterly_results.source_pdf_url),
            units = COALESCE(excluded.units, quarterly_results.units),
            stock_id = COALESCE(excluded.stock_id, quarterly_results.stock_id),
...
async def process_quarterly_results(ai_response, stock_symbol, message_id=None, pdf_url=None, exchange=None, announcement_date=None):
    ...
    try:
        ...
        # Apply sector-specific formula overrides if available
        try:
            async with open_analytics_db(readonly=True) as _sdb:
                sector_overrides = await _get_sector_formulas_for_stock(_sdb, stock_symbol)
            if sector_overrides:
                current_q = fy_eps["fy_eps_basic_standalone"].get("current_quarter") or fy_eps["fy_eps_basic_consolidated"].get("current_quarter")
                if current_q and current_q in sector_overrides:
                    sector_expr = sector_overrides[current_q]
                    # Build variables from all available period data
                    def _build_vars(periods_list):
                        v = {}
                        for p in periods_list:
                            q = p.get("quarter", "").upper()
                            if q and p.get("eps_diluted") is not None:
                                v[q] = p["eps_diluted"]
                            elif q and p.get("eps_basic") is not None:
                                v[q] = p["eps_basic"]
                            pt = p.get("period_type", "")
                            if pt == "six_month" and p.get("eps_diluted") is not None:
                                v["N6"] = p["eps_diluted"]
                            elif pt == "six_month" and p.get("eps_basic") is not None:
                                v["N6"] = p["eps_basic"]
                            if pt == "nine_month" and p.get("eps_diluted") is not None:
                                v["N9"] = p["eps_diluted"]
                            elif pt == "nine_month" and p.get("eps_basic") is not None:
                                v["N9"] = p["eps_basic"]
                            if pt == "annual":
                                if p.get("eps_diluted") is not None:
                                    v["FY"] = p["eps_diluted"]
                                elif p.get("eps_basic") is not None:
                                    v["FY"] = p["eps_basic"]
                        return v

                    if consolidated_periods:
                        c_vars = _build_vars(consolidated_periods)
                        val = _eval_formula_expr(sector_expr, c_vars)
                        if val is not None:
                            fy_diluted_c = val
                            fy_formula_c = f"SECTOR:{sector_expr}"
                            logger.info(f"Sector formula applied for {stock_symbol} (C): {sector_expr} = {val}")
                    if standalone_periods:
                        s_vars = _build_vars(standalone_periods)
                        val = _eval_formula_expr(sector_expr, s_vars)
                        if val is not None:
                            fy_diluted_s = val
                            fy_formula_s = f"SECTOR:{sector_expr}"
                            logger.info(f"Sector formula applied for {stock_symbol} (S): {sector_expr} = {val}")
        except Exception as sf_err:
            logger.warning(f"Sector formula lookup failed for {stock_symbol}: {sf_err}")
...
async def run_quarterly_extraction(pdf_url, stock_symbol, message_id=None, exchange=None, announcement_date=None):
    ...
    # Skip if a completed extraction with valid EPS already exists for this stock+quarter+day
    try:
        ann_date = announcement_date or get_ist_now().isoformat()
        quarter, fy = _quarter_fy_from_date(ann_date)
        ann_date_only = str(ann_date)[:10]
        async with open_analytics_db(readonly=True) as db:
            ...
            if existing:
                logger.info(f"Skipping extraction for {stock_symbol} ({quarter} {fy}) — completed row with EPS already exists")
                return None
    except Exception as e:
        logger.debug(f"Skip-check failed for {stock_symbol}, proceeding with extraction: {e}")
...
BSE_BOARD_MEETING_CSV = "bse_board_meeting_announcements.csv"
bse_board_meeting_csv_path = os.path.join("corporate_announcements", BSE_BOARD_MEETING_CSV)


async def _mark_bse_announcement_processed(scrip_code: str, announcement_type: str, pdf_url: str, status: int = 1):
    """Mark a BSE announcement as processed (1=success, 2=skipped/failed)."""
    try:
        now_iso = get_ist_now().isoformat()
        async with open_analytics_db() as db:
            await db.execute("""
                UPDATE bse_announcements_log SET processed = ?, processed_at = ?
                WHERE scrip_code = ? AND announcement_type = ? AND pdf_url = ?
            """, (status, now_iso, scrip_code, announcement_type, pdf_url))
            await db.commit()
    except Exception as e:
        logger.warning(f"Failed to mark BSE announcement processed: {e}")


async def _run_and_mark_bse_extraction(pdf_url, stock_symbol, message_id, exchange, announcement_date, scrip_code, announcement_type):
    """Run extraction and mark the bse_announcements_log row as processed."""
    try:
        result = await run_quarterly_extraction(pdf_url, stock_symbol, message_id, exchange=exchange, announcement_date=announcement_date)
        if result:
            await _mark_bse_announcement_processed(scrip_code, announcement_type, pdf_url, status=1)
        else:
            await _mark_bse_announcement_processed(scrip_code, announcement_type, pdf_url, status=2)
    except Exception as e:
        logger.error(f"BSE extraction wrapper error for {stock_symbol}: {e}")
        await _mark_bse_announcement_processed(scrip_code, announcement_type, pdf_url, status=2)


async def process_bse_results_data(bse_data):
    """Process BSE RESULT-category announcements — ensure ALL appear in PE page.
    Dedup via bse_announcements_log DB table (atomic, scalable, re-triggerable)."""
    if not bse_data or "Table" not in bse_data:
    ...
    for row in table:
        ...
        scrip_code = norm["symbol"]
        ann_date = norm.get("an_dt", "")

        # DB-based dedup: INSERT OR IGNORE — if row exists, skip
        is_new = False
        async with open_analytics_db() as db:
            await db.execute("""
                INSERT OR IGNORE INTO bse_announcements_log
                (scrip_code, company_name, announcement_type, announcement_date, subject, pdf_url, xml_url, exchange, processed, created_at)
                VALUES (?, ?, 'result', ?, ?, ?, ?, 'BSE', 0, ?)
            """, (scrip_code, norm["sm_name"], ann_date, norm["desc"],
                  attachment_file, norm.get("attchmntFile", ""), now_iso))
            await db.commit()
            if db.total_changes > 0:
                is_new = True

        if not is_new:
            # Check if it needs re-processing (processed=0 means pending retry)
            async with open_analytics_db(readonly=True) as db:
                pending = await db.execute(
                    "SELECT id FROM bse_announcements_log WHERE scrip_code = ? AND announcement_type = 'result' AND pdf_url = ? AND processed = 0",
                    (scrip_code, attachment_file))
                needs_retry = await pending.fetchone()
            if not needs_retry:
                continue

        # Dashboard/Telegram: only for new announcements
        message_id = None
        if is_new:
            new_count += 1
        ...
        # PE Page: ensure quarterly_results row exists
        if attachment_file == "-":
            await _insert_extraction_placeholder(
                norm["symbol"], norm["sm_name"], attachment_file,
                "BSE", ann_date, message_id,
                status='failed', error_msg='No attachment in BSE filing'
            )
            await _mark_bse_announcement_processed(scrip_code, 'result', attachment_file, status=2)
        else:
            await _insert_extraction_placeholder(
                norm["symbol"], norm["sm_name"], attachment_file,
                "BSE", ann_date, message_id
            ...
            asyncio.create_task(
                _run_and_mark_bse_extraction(
                    attachment_file, norm["symbol"], message_id, "BSE", ann_date, scrip_code, 'result'
                )
            )

    logger.info(f"BSE results: {len(table)} total announcements, {new_count} new for dashboard/PE")
    return 1


def _fetch_bse_board_meeting_sync():
    """Sync BSE fetch — only BOARD_MEETING category announcements for today, paginated."""
    bse_download_folder.mkdir(parents=True, exist_ok=True)
    today = datetime.now()
    all_rows = []
    with BSE(bse_download_folder) as bse:
        ...
        while page <= max_pages:
            try:
                data = bse.announcements(
                    page_no=page, from_date=today, to_date=today,
                    segment="equity", category=BSE_CATEGORY.BOARD_MEETING,
                )
                table = data.get("Table", [])
                if not table:
                    break
                ...
                logger.warning(f"BSE board meeting page {page} fetch error: {e}")
                break
    logger.info(f"BSE BOARD_MEETING fetch: {len(all_rows)} announcements across {page} page(s) for {today.strftime('%Y-%m-%d')}")
    return {"Table": all_rows}


async def fetch_bse_board_meeting():
    """Fetch BSE BOARD_MEETING-category announcements only (runs sync BSE in executor)."""
    try:
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, _fetch_bse_board_meeting_sync)
    except Exception as e:
        logger.error(f"BSE board meeting fetch failed: {e}")
        return None


async def process_bse_board_meeting_data(bse_data):
    """Process BSE BOARD_MEETING-category announcements — filter for 'financial result' keyword,
    then trigger PE extraction for matched items. Dedup via bse_announcements_log DB table."""
    if not bse_data or "Table" not in bse_data:
        logger.warning("BSE board meeting data empty or invalid structure")
        return None
    table = bse_data["Table"]
    if not table:
        return 1
    ...
    for row in table:
        ...
        if "financial result" not in searchable:
            continue
        if "board meeting intimation" in searchable:
            skipped_intimation += 1
            continue
        financial_matched.append(row)

    if not financial_matched:
        logger.info(f"BSE board meeting: {len(table)} total, 0 matched after filter (skipped {skipped_intimation} intimations)")
        return 1

    now_iso = get_ist_now().isoformat()
    new_count = 0

    for row in financial_matched:
        norm = _bse_row_to_normalized(row)
        attachment_file = norm["attchmntFile"]
        scrip_code = norm["symbol"]
        ann_date = norm.get("an_dt", "")
        ...
        if not is_new:
            async with open_analytics_db(readonly=True) as db:
                pending = await db.execute(
                    "SELECT id FROM bse_announcements_log WHERE scrip_code = ? AND announcement_type = 'board_meeting' AND pdf_url = ? AND processed = 0",
                    (scrip_code, attachment_file))
                needs_retry = await pending.fetchone()
            if not needs_retry:
                continue
        ...
        if attachment_file == "-":
            await _insert_extraction_placeholder(
                norm["symbol"], norm["sm_name"], attachment_file,
                "BSE", ann_date, message_id,
                status='failed', error_msg='No attachment in BSE board meeting filing'
            )
            await _mark_bse_announcement_processed(scrip_code, 'board_meeting', attachment_file, status=2)
        else:
            await _insert_extraction_placeholder(
                norm["symbol"], norm["sm_name"], attachment_file,
                "BSE", ann_date, message_id
            ...
            asyncio.create_task(
                _run_and_mark_bse_extraction(
                    attachment_file, norm["symbol"], message_id, "BSE", ann_date, scrip_code, 'board_meeting'
                )
            )

    logger.info(f"BSE board meeting: {len(table)} total, {len(financial_matched)} matched, {skipped_intimation} intimations skipped, {new_count} new for dashboard/PE")
    return 1
...
async def CA_bse_board_meeting():
    """Fetch BSE BOARD_MEETING-category announcements and trigger quarterly extraction for financial result items."""
    try:
        bse_data = await fetch_bse_board_meeting()
        if bse_data:
            return await process_bse_board_meeting_data(bse_data)
    except Exception as e:
        logger.error(f"BSE Board Meeting CA error: {e}")
    return None


async def run_periodic_task_bse_board_meeting():
    """Background Job 4: BSE BOARD_MEETING-category — starts at +50s, runs every 60s.
    Fetches board meeting announcements, filters for 'financial result' keyword, triggers PE extraction."""
    global ai_processing_active
    logger.info("Starting background job: BSE board meeting (offset=50s, interval=60s)")
    ...
    while True:
        try:
            if ai_processing_active:
                await asyncio.sleep(10)
                continue
            bse_result = await CA_bse_board_meeting()
            if bse_result is None:
                logger.warning("BSE board meeting fetch failed, will retry next cycle")
            else:
                logger.info("BSE board meeting fetch completed")
            await asyncio.sleep(60)
        except Exception as e:
            logger.error(f"Error in BSE board meeting background job: {str(e)}")
            await asyncio.sleep(30)
...
async def receive_trigger_message(message_data: MessageData):
    ...
    try:
        ...
        ws_msg = {"type": "new_message", "message": message_data.dict()}
        await _enqueue_message_write(params, ws_msg)
...
async def get_pe_analysis(
    fetch_cmp: bool = False, symbols: str = "",
    valuation_filter: str = "", page: int = 0, limit: int = 0,
    search: str = "", year: str = "", quarter: str = "",
    ...
    Server-side filtering: valuation_filter (pending/reviewed), page, limit, search, year, quarter, exchange, sector, date_from, date_to."""
    try:
        import time as _time
        _t0 = _time.perf_counter()

        # --- Phase 1: Main query (single connection, then close) ---
        extra_where = ["qr.quarter NOT IN ('FY')"]
        extra_params = []

        if valuation_filter == "pending":
            extra_where.append("(qr.valuation IS NULL OR qr.valuation = '')")
        elif valuation_filter == "reviewed":
            extra_where.append("(qr.valuation IS NOT NULL AND qr.valuation != '')")

        if search:
            like = f"%{search}%"
            extra_where.append("(qr.stock_symbol LIKE ? OR qr.company_name LIKE ? OR s.sector LIKE ?)")
            extra_params.extend([like, like, like])

        if year:
            yr_list = [y.strip() for y in year.split(",") if y.strip()]
            if yr_list:
                fy_conds = []
                for y_val in yr_list:
                    fy_conds.append("qr.financial_year LIKE ?")
                    extra_params.append(f"%{y_val[-2:]}")
                extra_where.append(f"({' OR '.join(fy_conds)})")

        if quarter:
            q_list = [q.strip().upper() for q in quarter.split(",") if q.strip()]
            if q_list:
                ph = ",".join(["?"] * len(q_list))
                extra_where.append(f"UPPER(qr.quarter) IN ({ph})")
                extra_params.extend(q_list)

        if exchange:
            e_list = [e.strip() for e in exchange.split(",") if e.strip()]
            if e_list:
                ph = ",".join(["?"] * len(e_list))
                extra_where.append(f"COALESCE(s.exchange, qr.exchange) IN ({ph})")
                extra_params.extend(e_list)

        if sector:
            s_list = [s.strip() for s in sector.split(",") if s.strip()]
            if s_list:
                ph = ",".join(["?"] * len(s_list))
                extra_where.append(f"s.sector IN ({ph})")
                extra_params.extend(s_list)

        if date_from:
            extra_where.append("COALESCE(qr.announcement_date, qr.updated_at) >= ?")
            extra_params.append(date_from)
        if date_to:
            extra_where.append("COALESCE(qr.announcement_date, qr.updated_at) <= ?")
            extra_params.append(date_to + "T23:59:59.999")

        where_clause = " AND ".join(extra_where)

        _pe_db = await aiosqlite.connect(ANALYTICS_DB_PATH)
        for _p in _DB_PRAGMAS:
            await _pe_db.execute(_p)
        await _pe_db.execute("PRAGMA query_only=ON")
        _pe_db.row_factory = aiosqlite.Row
        try:
            cursor = await _pe_db.execute(f"""
                SELECT
                    qr.stock_symbol, qr.company_name, qr.quarter, qr.financial_year,
                    ...
                    CASE WHEN qr.fy_eps_diluted_consolidated IS NOT NULL THEN qr.fy_eps_formula_consolidated
                         WHEN qr.fy_eps_basic_consolidated IS NOT NULL THEN qr.fy_eps_formula_consolidated
                         ELSE qr.fy_eps_formula_standalone END AS fy_eps_formula,
                    CASE WHEN qr.fy_eps_diluted_consolidated IS NOT NULL THEN 'C'
                         WHEN qr.fy_eps_basic_consolidated IS NOT NULL THEN 'C'
                         ELSE 'S' END AS eps_basis,
                    qr.units, qr.updated_at, qr.cmp AS stored_cmp, qr.pe AS stored_pe,
                    qr.cmp_updated_at,
                    COALESCE(qr.source_pdf_url, qr.source_pdf_url_tracking) AS source_pdf_url,
                    qr.valuation, qr.comments, qr.announcement_date,
                    s.sector, COALESCE(s.exchange, qr.exchange) AS exchange,
                    COALESCE(qr.extraction_status, 'completed') AS extraction_status,
                    qr.extraction_error, qr.recommendation, qr.target_price,
                    qr.manual_fy_eps, qr.manual_fy_eps_formula, s.sub_sector, qr.created_at
                FROM quarterly_results qr
        ...
        logger.info(f"PE Analysis: SQL query {len(all_rows)} raw rows in {(_t1-_t0)*1000:.0f}ms")

        # --- Phase 2: Python filtering (no DB needed) ---
        best_per_stock = {}
        for r in all_rows:
            sym = r["stock_symbol"]
            ext = r["extraction_status"] or "completed"
            key = (r["financial_year"] or "", r["quarter"] or "")
            if sym not in best_per_stock:
                best_per_stock[sym] = (key, ext, r)
            else:
                prev_key, prev_ext, _ = best_per_stock[sym]
                # Prefer completed over non-completed; then latest FY+Quarter
                if (ext == "completed" and prev_ext != "completed") or key > prev_key:
                    best_per_stock[sym] = (key, ext, r)

        filtered_rows = [entry[2] for entry in best_per_stock.values()]
        ...
        logger.info(f"PE Analysis: filtered to {len(rows)} rows (total {total_count}) in {(_t_filter-_t1)*1000:.0f}ms")
        ...
        for sym in sym_set:
            ...
            if not norm_yr:
                continue
            yr_str = str(norm_yr)
            sym_fy_variants[sym] = {yr_str, f"FY{norm_yr - 1}-{yr_str[-2:]}", f"FY{yr_str[-2:]}"}
            prev_yr = norm_yr - 1
            prev_yr_str = str(prev_yr)
            sym_pfy_variants[sym] = {prev_yr_str, f"FY{prev_yr - 1}-{prev_yr_str[-2:]}", f"FY{prev_yr_str[-2:]}"}

        # --- Phase 3: Quarters query (reuses _pe_db connection) ---
        prev_fy_map = {}
        if sym_set:
            ...
            fy_variants_all = set()
            for s in sym_list:
                fy_variants_all.update(sym_fy_variants.get(s, set()))
                fy_variants_all.update(sym_pfy_variants.get(s, set()))
            fy_list = list(fy_variants_all)
            fy_ph = ','.join(['?'] * len(fy_list)) if fy_list else "''"
            fy_clause = f" AND financial_year IN ({fy_ph})" if fy_list else ""

            logger.info(f"PE Analysis: starting quarters query for {len(sym_set)} symbols, {len(fy_list)} FY variants")
            all_qr_cursor = await _pe_db.execute(f"""
                ...
                FROM quarterly_results
                WHERE stock_symbol IN ({placeholders}){fy_clause}
            """, sym_list + fy_list)
            qr_rows = await all_qr_cursor.fetchall()

            logger.info(f"PE Analysis: quarters query fetched {len(qr_rows)} rows")
            seen_cur = {}
            seen_prev = {}
            for qr in qr_rows:
                sym = qr["stock_symbol"]
                q_label = qr["quarter"]
                fy_val = qr["financial_year"]

                if sym in sym_fy_variants and fy_val in sym_fy_variants[sym]:
                    if sym not in seen_cur:
                        seen_cur[sym] = set()
                    if q_label not in seen_cur[sym]:
                        seen_cur[sym].add(q_label)
                ...
                if sym in sym_pfy_variants and fy_val in sym_pfy_variants[sym]:
                    if sym not in seen_prev:
                        seen_prev[sym] = set()
                    if q_label not in seen_prev[sym]:
                        seen_prev[sym].add(q_label)
                        if sym not in prev_fy_map:
                            prev_fy_map[sym] = {}
                        if q_label == "FY" and qr["qtr_eps"] is not None:
                            prev_fy_map[sym]["PFY"] = round(qr["qtr_eps"], 4)
                        if q_label in ("Q1", "Q2", "Q3", "Q4") and qr["qtr_eps"] is not None:
                            prev_fy_map[sym][f"P{q_label}"] = round(qr["qtr_eps"], 4)
                        if q_label == "Q1" and qr["cum_eps"] is not None:
                            prev_fy_map[sym]["PN3"] = round(qr["cum_eps"], 4)
                        if q_label == "Q2" and qr["cum_eps"] is not None:
                            prev_fy_map[sym]["PN6"] = round(qr["cum_eps"], 4)
                        if q_label == "Q3" and qr["cum_eps"] is not None:
                            prev_fy_map[sym]["PN9"] = round(qr["cum_eps"], 4)

        await _pe_db.close()

        _t2 = _time.perf_counter()
        logger.info(f"PE Analysis: query+quarters in {(_t2-_t0)*1000:.0f}ms")

        # --- Phase 4: Build response (no DB needed) ---
        for sym in sym_set:
            if sym not in quarters_map:
                quarters_map[sym] = {}
            qm = quarters_map[sym]
        ...
        for r in rows:
            ...
            manual_eps = r["manual_fy_eps"] if "manual_fy_eps" in r.keys() else None
            manual_eps_rounded = round(manual_eps, 2) if manual_eps is not None else None
            manual_formula = r["manual_fy_eps_formula"] if "manual_fy_eps_formula" in r.keys() else None

            # PE uses manual_fy_eps if present, else auto fy_eps
            effective_fy_eps = manual_eps_rounded if manual_eps_rounded else fy_eps_rounded
            pe_val = stored_pe
            if stored_cmp and effective_fy_eps and effective_fy_eps > 0 and not stored_pe:
                pe_val = round(stored_cmp / effective_fy_eps, 2)
            ...
            stock_data.append({
                ...
                "manual_fy_eps": manual_eps_rounded,
                "manual_fy_eps_formula": manual_formula,
                "fy_eps_formula": f"{fy_formula or ''} ({eps_basis})".strip(),
                "eps_basis": eps_basis,
                "units": r["units"],
                "sector": r["sector"],
                "sub_sector": r["sub_sector"] if "sub_sector" in r.keys() else None,
                "exchange": r["exchange"],
                ...
                "target_price": round(r["target_price"], 2) if ("target_price" in r.keys() and r["target_price"] is not None) else None,
            })
            if fetch_cmp:
        ...
        logger.info(f"PE Analysis: built {len(stock_data)} rows in {(_t3-_t2)*1000:.0f}ms, total {(_t3-_t0)*1000:.0f}ms")
                ...
                async def _fetch_cmp_nse(syms):
                    ...
                    for q in flattened:
                        ...
                        if close_price and token_val in token_to_symbol:
                            result_map[token_to_symbol[token_val]] = float(close_price)
                ...
                async def _fetch_cmp_bse(syms):
                    ...
                    for q in flattened:
                        ...
                        if close_price and matched_sym:
                            result_map[matched_sym] = float(close_price)
        ...
        for item in stock_data:
            ...
            if live_cmp:
                ...
                eff_eps = item.get("manual_fy_eps") or item.get("fy_eps")
                if eff_eps and eff_eps > 0:
                    item["pe"] = round(live_cmp / eff_eps, 2)

        total_pages = (total_count + limit - 1) // limit if limit > 0 else 1
        resp = {
            "success": True, "results": stock_data, "total": len(stock_data),
        ...
        if '_pe_db' in locals():
            try: await _pe_db.close()
            except Exception: pass
        logger.error(f"Error in PE analysis: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/pe_analysis/filter_options")
async def get_pe_filter_options(valuation_filter: str = ""):
    """Return available filter values (years, quarters, exchanges, sectors) for PE Analysis dropdowns."""
    try:
        async with open_analytics_db(readonly=True) as db:
            db.row_factory = aiosqlite.Row
            val_clause = ""
            if valuation_filter == "pending":
                val_clause = " AND (qr.valuation IS NULL OR qr.valuation = '')"
            elif valuation_filter == "reviewed":
                val_clause = " AND (qr.valuation IS NOT NULL AND qr.valuation != '')"
            cursor = await db.execute(f"""
                SELECT DISTINCT qr.financial_year, qr.quarter,
                    COALESCE(s.exchange, qr.exchange) AS exchange, s.sector
                FROM quarterly_results qr
                LEFT JOIN stocks s ON s.symbol = qr.stock_symbol
                WHERE qr.quarter NOT IN ('FY'){val_clause}
            """)
            rows = await cursor.fetchall()
            years = set()
            quarters = set()
            ...
            for r in rows:
                ...
                norm = _normalize_fy_year(fy)
                if norm:
                    years.add(str(norm))
                if r["quarter"]:
                    quarters.add(r["quarter"].upper())
                if r["exchange"]:
        ...
        logger.error(f"Error fetching PE filter options: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/pe_analysis/report_summary")
async def get_pe_report_summary():
    """Aggregated analytics summary from PE Reviewed data. All computation is server-side."""
    try:
        async with open_analytics_db(readonly=True) as db:
            db.row_factory = aiosqlite.Row
            ...
            cursor = await db.execute("""
                SELECT
                    ...
                    qr.manual_fy_eps,
                    qr.cmp, qr.pe,
                    qr.valuation, qr.recommendation, qr.target_price,
                ...
                WHERE qr.quarter NOT IN ('FY')
                  AND qr.valuation IS NOT NULL AND qr.valuation != ''
                  AND COALESCE(qr.extraction_status, 'completed') = 'completed'
                ORDER BY qr.created_at DESC
            """)
            all_rows = await cursor.fetchall()
        ...
        if total == 0:
            return {"success": True, "summary": {
                "total_reviewed": 0, "valuation_counts": {}, "recommendation_counts": {},
                "avg_pe": None, "median_pe": None,
                "pe_distribution": [], "sector_summary": [], "top10_cheapest": [], "top10_expensive": [],
            }}

        val_counts = {}
        rec_counts = {"BUY": 0, "SELL": 0, "HOLD": 0, "NONE": 0}
        pe_values = []
        sector_data = {}
        pe_stocks = []

        for r in rows:
            v = (r.get("valuation") or "").upper()
            ...
            eff_eps = r.get("manual_fy_eps") if r.get("manual_fy_eps") is not None else r.get("fy_eps")
            cmp = r.get("cmp")
            pe = r.get("pe")
            if pe is None and eff_eps and eff_eps > 0 and cmp and cmp > 0:
                pe = round(cmp / eff_eps, 2)

            if pe is not None and pe > 0:
                pe_values.append(pe)

            sect = r.get("sector") or "Unknown"
            if sect not in sector_data:
                sector_data[sect] = {"count": 0, "pe_vals": [], "buy": 0, "sell": 0, "hold": 0, "none": 0, "cheap": 0, "expensive": 0, "other": 0}
            sd = sector_data[sect]
            sd["count"] += 1
            if pe is not None and pe > 0:
                sd["pe_vals"].append(pe)
            if rec == "BUY": sd["buy"] += 1
            elif rec == "SELL": sd["sell"] += 1
            elif rec == "HOLD": sd["hold"] += 1
            else: sd["none"] += 1
            if v == "CHEAP": sd["cheap"] += 1
            elif v == "EXPENSIVE": sd["expensive"] += 1
            else: sd["other"] += 1

            if pe is not None and pe > 0:
                upside = None
                tp = r.get("target_price")
                if tp and cmp and cmp > 0:
                    upside = round((tp - cmp) / cmp * 100, 1)
                pe_stocks.append({
                    "symbol": r.get("stock_symbol", ""),
                    "company_name": r.get("company_name", ""),
                    "pe": round(pe, 2),
                    "cmp": round(cmp, 2) if cmp else None,
                    "fy_eps": round(eff_eps, 2) if eff_eps else None,
                    "sector": sect,
        ...
        median_pe = round(pe_values[len(pe_values) // 2], 2) if pe_values else None

        buckets = [(0, 10), (10, 20), (20, 30), (30, 50), (50, 100), (100, float('inf'))]
        bucket_labels = ["0-10", "10-20", "20-30", "30-50", "50-100", "100+"]
        pe_dist = []
        for (lo, hi), label in zip(buckets, bucket_labels):
            cnt = sum(1 for p in pe_values if lo <= p < hi)
            pe_dist.append({"range": label, "count": cnt})

        sector_summary = []
        for sect, sd in sorted(sector_data.items(), key=lambda x: x[1]["count"], reverse=True):
            s_avg = round(sum(sd["pe_vals"]) / len(sd["pe_vals"]), 2) if sd["pe_vals"] else None
            sector_summary.append({
                "sector": sect, "count": sd["count"], "avg_pe": s_avg,
                "buy": sd["buy"], "sell": sd["sell"], "hold": sd["hold"], "none": sd["none"],
                "cheap": sd["cheap"], "expensive": sd["expensive"], "other": sd["other"],
            })

        pe_stocks.sort(key=lambda x: x["pe"])
        top10_cheapest = pe_stocks[:10]
        top10_expensive = list(reversed(pe_stocks[-10:])) if len(pe_stocks) >= 10 else list(reversed(pe_stocks))
        ...
        return {"success": True, "summary": {
            ...
            "top10_expensive": top10_expensive,
        }}
    except Exception as e:
        logger.error(f"Error in PE report summary: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
class PEEditRequest(BaseModel):
    ...
    manual_fy_eps: float = None
    manual_fy_eps_formula: str = None
    save_as_sector_default: bool = False
    ...
    try:
        async with open_analytics_db() as db:
            ...
            if body.recommendation is not None:
                ...
                params.append(body.recommendation if body.recommendation else None)

            if body.target_price is not None:
                sets.append("target_price = ?")
                params.append(body.target_price)

            if body.manual_fy_eps is not None:
                sets.append("manual_fy_eps = ?")
                params.append(body.manual_fy_eps)

            if body.manual_fy_eps_formula is not None:
                sets.append("manual_fy_eps_formula = ?")
                params.append(body.manual_fy_eps_formula if body.manual_fy_eps_formula else None)

            pe_val = None
            cmp_for_pe = body.cmp
            fy_for_pe = body.manual_fy_eps if body.manual_fy_eps else body.fy_eps
            if cmp_for_pe and fy_for_pe and fy_for_pe > 0:
                pe_val = round(cmp_for_pe / fy_for_pe, 2)
                sets.append("pe = ?")
                params.append(pe_val)
            elif body.cmp is not None or body.fy_eps is not None or body.manual_fy_eps is not None:
                row = await db.execute(
                    "SELECT cmp, COALESCE(manual_fy_eps, fy_eps_diluted_consolidated, fy_eps_basic_consolidated, fy_eps_diluted_standalone, fy_eps_basic_standalone) as fy FROM quarterly_results WHERE stock_symbol = ? AND quarter = ? AND financial_year = ?",
                    (stock_symbol, match_q, match_fy)
                )
                existing = await row.fetchone()
                if existing:
                    c = body.cmp if body.cmp is not None else (existing[0] or 0)
                    f = body.manual_fy_eps if body.manual_fy_eps is not None else (body.fy_eps if body.fy_eps is not None else (existing[1] or 0))
                    if c and f and f > 0:
                        pe_val = round(c / f, 2)
                        sets.append("pe = ?")
                        params.append(pe_val)
            ...
            if body.sub_sector is not None:
                await db.execute(
                    "UPDATE stocks SET sub_sector = ?, updated_at = ? WHERE symbol = ?",
                    (body.sub_sector if body.sub_sector else None, now_iso, stock_symbol)
                )

            # Upsert sector_formulas if user confirmed save as sector default
            if body.save_as_sector_default and body.manual_fy_eps_formula:
                import json as _json
                sector_val = body.sector
                if not sector_val:
                    sc = await db.execute("SELECT sector FROM stocks WHERE symbol = ?", (stock_symbol,))
                    sr = await sc.fetchone()
                    sector_val = sr[0] if sr else None
                if sector_val:
                    try:
                        formula_obj = _json.loads(body.manual_fy_eps_formula)
                    except (ValueError, TypeError):
                        formula_obj = {}
                    sub_sec = body.sub_sector.strip() if body.sub_sector else ''
                    for q_key in ("Q1", "Q2", "Q3", "Q4"):
                        expr = formula_obj.get(f"{q_key.lower()}_expr") or formula_obj.get(f"{q_key}_expr")
                        if expr:
                            await db.execute("""
                                ...
                                DO UPDATE SET formula_expr = excluded.formula_expr, updated_at = excluded.updated_at
                            """, (sector_val, sub_sec, q_key, expr.strip(), now_iso, now_iso))
...
async def get_pe_sectors():
    """Distinct sector + sub_sector list from stocks table for PE edit dropdown."""
    try:
        async with open_analytics_db(readonly=True) as db:
            cursor = await db.execute(
            ...
            cursor2 = await db.execute(
                "SELECT DISTINCT sector, sub_sector FROM stocks WHERE sub_sector IS NOT NULL AND TRIM(sub_sector) != '' ORDER BY sector, sub_sector"
            )
            sub_rows = await cursor2.fetchall()
            sub_sectors = {}
            for r in sub_rows:
                sub_sectors.setdefault(r[0], []).append(r[1])
            return {"success": True, "sectors": sectors, "sub_sectors": sub_sectors}
    except Exception as e:
        logger.error(f"Error fetching PE sectors: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/sector_formulas")
async def get_sector_formulas():
    """Return all sector-level formula overrides, grouped by sector/sub_sector."""
    try:
        async with open_analytics_db(readonly=True) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute("SELECT * FROM sector_formulas ORDER BY sector, sub_sector, quarter")
            rows = [dict(r) for r in await cursor.fetchall()]
            grouped = {}
            for r in rows:
                sub = r['sub_sector'] if r['sub_sector'] else None
                key = f"{r['sector']}|{sub or ''}"
                if key not in grouped:
                    grouped[key] = {"sector": r["sector"], "sub_sector": sub, "formulas": {}}
                grouped[key]["formulas"][r["quarter"]] = r["formula_expr"]
            return {"success": True, "sector_formulas": list(grouped.values())}
    except Exception as e:
        logger.error(f"Error fetching sector formulas: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        ...
        async def run_get_quotes_background():
            try:
                ...
                # Step 4: Process results (70% progress)
                ...
                # Step 5: Update DataFrame (75% progress)
                ...
                # Step 5b: Retry stocks that returned open=0 (market not yet open for them)
                missing_count = sum(1 for idx in valid_indices if pd.isna(df.at[idx, 'OPEN PRICE']))
                if missing_count > 0:
                    active_jobs[job_id].message = f"Retrying {missing_count} stocks with no price (waiting for market data)..."
                    ...
                    df, retried_count = await retry_missing_quotes(df, valid_indices, max_retries=2, retry_delay=120)
                    logger.info(f"[Job {job_id}] Retry filled {retried_count} additional prices")
                ...
                active_jobs[job_id].progress = 100
                final_missing = sum(1 for idx in valid_indices if pd.isna(df.at[idx, 'OPEN PRICE']))
                msg = f"Quotes fetched - {total_symbols - final_missing}/{total_symbols} stocks updated"
                if final_missing > 0:
                    msg += f" ({final_missing} still missing)"
                active_jobs[job_id].message = msg
                active_jobs[job_id].completed_at = get_ist_now().isoformat()
                active_jobs[job_id].result = {"status": "success", "stocks_processed": total_symbols, "missing": final_missing}
